...
class HealthCareChatbot:
    ...
    def load_data(self, data_file: str = None) -> List[Dict]:
        ...
        if not file_path or not os.path.exists(file_path):
            raise FileNotFoundError(
                ...
                "먼저 collect_data()를 실행하여 데이터를 수집하세요."
            )
    ...
    def build_vector_store(self) -> FAISS:
        ...
        splitter = RecursiveCharacterTextSplitter(
            chunk_size=500, chunk_overlap=100, separators=["\n\n", "\n", " ", ""]  ## chunk_size, chunk_overlap 500, 100 권장 값  
        )
        chunks = splitter.split_documents(documents)
        print(f"  → {len(chunks)}개 청크로 분할 완료")

        # 임베딩 및 벡터 스토어 생성 (OpenAI Embeddings 사용)
        print("  → 임베딩 모델 로딩 중...")
        openai_embeddings = OpenAIEmbeddings(model=self.embedding_model_name)
        
        self.vector_store = FAISS.from_documents(chunks, openai_embeddings)
        # 로컬 저장
    ...
    def load_vector_store(self) -> FAISS:
        ...
        try:
            ...
            self.vector_store = FAISS.load_local(
                self.faiss_dir, openai_embeddings, allow_dangerous_deserialization=True
            )
    ...
    def add_documents_to_vector_store(self, new_data_file: str) -> FAISS:
        """
        기존 벡터 스토어에 새로운 JSON 파일의 문서를 추가합니다.
        
        Args:
            new_data_file: 추가할 JSON 파일 경로
        
        Returns:
            업데이트된 FAISS 벡터 스토어
        """
        if not new_data_file or not os.path.exists(new_data_file):
            raise FileNotFoundError(f"파일을 찾을 수 없습니다: {new_data_file}")
        
        print("\n" + "=" * 80)
        print("📝 벡터 스토어에 새 문서 추가 중...")
        print("=" * 80)
        
        # 기존 벡터 스토어 로드 (없으면 새로 생성)
        if not self.vector_store:
            loaded = self.load_vector_store()
            if loaded is None:
                raise ValueError(
                    "기존 벡터 스토어가 없습니다. 먼저 initialize()를 실행하거나 "
                    "build_vector_store()를 실행하세요."
                )
        
        # 새 JSON 파일 로드
        print(f"\n  → 새 데이터 파일 로드: {new_data_file}")
        with open(new_data_file, "r", encoding="utf-8") as f:
            new_data = json.load(f)
        
        print(f"  → {len(new_data)}개 새 문서 로드 완료")
        ...
        print(f"  → {len(new_documents)}개 새 문서 준비 완료")
        ...
        print(f"  → {len(new_chunks)}개 새 청크로 분할 완료")
        
        # 기존 벡터 스토어에 추가
        print("  → 임베딩 생성 및 벡터 스토어에 추가 중...")
        openai_embeddings = OpenAIEmbeddings(model=self.embedding_model_name)
        ...
        print(f"✅ 벡터 스토어에 {len(new_chunks)}개 청크 추가 완료")
        print(f"✅ 저장 위치: {self.faiss_dir}\n")
        
        # 기존 structured_data에도 추가 (선택사항)
        if hasattr(self, 'structured_data') and self.structured_data:
            self.structured_data.extend(new_data)
    ...
    def setup_agent(self):
        ...
        #     """
        #     건강 지원 정보 데이터베이스에서 관련 정보를 검색합니다.
        #     """
        #     try:
        ...
        #                 f"지원대상: {meta.get('support_target', 'N/A')}\n"
...
지침:
- 사용자의 질문에 대해 검색 도구를 사용하여 관련 정보를 찾으세요.
- 검색 결과를 바탕으로 명확하고 친절하게 답변하세요.
- 지원 대상, 지원 내용, 신청 방법 등 핵심 정보를 간결하게 요약하세요.
- 여러 지역의 정보가 있다면 지역별로 구분하여 안내하세요. 만약 제공된 문서에 세부 지원 내용이 존재한다면 그 내용을 기반으로 답변할것
- 정보가 부족하면 "해당 정보를 찾을 수 없습니다"라고 솔직히 말하세요.
- 예시로 질문 : 암 지원에 대해 알려줘 인 경우 제공 문서에 암 지원이 없으면 참조하지마
...
"""
    ...
    def initialize(
        ...
        Args:
            start_url: 데이터 수집할 URL (data_file이 없을 때 필요)
        ...
        if data_file and os.path.exists(data_file):
            ...
            raise ValueError(
                "start_url 또는 data_file 중 하나는 필수입니다.\n"
                "새로 수집하려면 start_url을, 기존 데이터를 사용하려면 data_file을 제공하세요."
            )
...
def main():
    ...
    if not args.url and not args.data_file:
        ...
        if choice == "1":
            ...
            new_data_file = input("추가할 JSON 파일 경로: ").strip()
            
            if not new_data_file or not os.path.exists(new_data_file):
                print(f"❌ 파일을 찾을 수 없습니다: {new_data_file}")
                return
            ...
            try:
                ...
                # 기존 벡터 스토어 로드
                if chatbot.load_vector_store() is None:
                    print("❌ 기존 벡터 스토어가 없습니다. 먼저 옵션 1 또는 2로 초기화하세요.")
                    return
                
                # 새 문서 추가
                chatbot.add_documents_to_vector_store(new_data_file)
                
                # 에이전트 재설정
                chatbot.setup_agent()
                
                print("\n✅ 새 문서 추가 완료! 대화를 시작합니다.")
