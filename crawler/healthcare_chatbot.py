...
#                 raise ImportError("잘못된 fitz 패키지입니다.")
#         except ImportError:
#             raise ImportError(
#                 "PyMuPDF가 설치되지 않았습니다. 다음 명령어로 설치하세요: pip install pymupdf"
#             )
# except Exception as e:
#     if isinstance(e, ImportError):
#         raise
#     raise ImportError(
#         f"PyMuPDF import 중 오류 발생: {e}\n"
#         "다른 fitz 패키지와 충돌할 수 있습니다. 다음을 시도하세요:\n"
...
class HealthCareChatbot:
    ...
    def load_pdf(self, pdf_file: str, force_reload: bool = False) -> str:
        """
        PDF 파일에서 텍스트 추출 및 구조화된 데이터 생성 (PyMuPDF 사용)

        Args:
            pdf_file: PDF 파일 경로
            force_reload: True면 기존 JSON 파일 무시하고 재처리
        ...
        output_json = os.path.join(
            self.output_dir, f"pdf_{pdf_name}_{timestamp}.json"
        )

        # 기존 파일이 있고 재처리 안 하면 건너뛰기
        if os.path.exists(output_json) and not force_reload:
            print(f"✓ 기존 PDF 데이터 사용: {output_json}")
            self.data_file = output_json
            return output_json
        ...
        try:
            ...
            # 페이지별로 아이템 생성 (각 페이지를 별도 아이템으로 처리)
            structured_items = []
            for page_num in range(total_pages):
                ...
                # PDF 파일명과 페이지 번호를 제목으로 사용
                title = f"{pdf_name} - 페이지 {page_num + 1}"

                # structured_data 형식으로 변환
                structured_item = {
                    "id": f"pdf_{pdf_name}_{timestamp}_p{page_num + 1}",
                    "title": title,
                    "raw_text": page_text,
                    "source_url": f"file://{os.path.abspath(pdf_file)}#page={page_num + 1}",
                    "region": self.region or "미지정",
                    "support_target": "",
                    "support_content": "",
            ...
            # 리스트로 변환 (기존 형식과 호환)
            self.structured_data = structured_items
            
            print(f"  → {len(structured_items)}개 페이지 처리 완료")
            ...
            self.data_file = output_json
            print(f"✅ PDF 처리 완료: {output_json}")
            if structured_items:
                total_text_length = sum(len(item.get("raw_text", "")) for item in structured_items)
                print(f"  → 총 추출된 텍스트 길이: {total_text_length:,}자")
                print(f"  → 페이지별 평균 텍스트 길이: {total_text_length // len(structured_items):,}자")
    ...
    def build_vector_store(self) -> FAISS:
        ...
        # # 청킹 전략 분기
        # if self.chunk_strategy == "per_item":
        #     chunks = documents
        #     print(f"  → 문서 단위 {len(chunks)}개 청크 생성(per_item)")
        # elif self.chunk_strategy == "by_fields":
        #     sectioned_docs = []
        #     for item in self.structured_data:
        #         base_meta = {
        #             "id": item.get("id", ""),
        ...
        #         }
        #         parts = [
        #             ("지원대상", item.get("support_target", "")),
        #             ("지원내용", item.get("support_content", "")),
        #             ("원문", item.get("raw_text", "")),
        #         ]
        #         for name, content in parts:
        #             if content:
        #                 meta = {**base_meta, "section": name}
        #                 sectioned_docs.append(Document(page_content=content, metadata=meta))
        #     chunks = sectioned_docs
        #     print(f"  → 섹션 단위 {len(chunks)}개 청크 생성(by_fields)")
        # else:
    ...
    def setup_agent(self):
        ...
        def search_with_score(query: str) -> str:
            ...
            """
            try:
                # 대화 시 설정된 지역명이 있으면 필터 적용
                search_filter = None
                if self.search_region:
                    search_filter = {"region": self.search_region}
...
"""
    ...
    async def run_conversation(self):
        ...
        print("📍 지역 정보 입력")
        print("=" * 80)
        region_input = await asyncio.to_thread(
            input, 
            "조회할 지역명을 입력하세요 (예: 강남구, 동작구, Enter: 전체 지역): "
        )
        if region_input and region_input.strip():
            self.search_region = region_input.strip()
            print(f"지역 필터 설정: {self.search_region}")
        else:
